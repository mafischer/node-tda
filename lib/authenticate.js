...
exports.init = async function init({ key = undefined, cert = undefined }, authCb) {
...
exports.authenticate = function authenticate({ consumerKey, key = undefined, cert = undefined }, callback = undefined) {

  const redirectURI = 'https://localhost:8443';
  const clientId = encodeURI(`${consumerKey}@AMER.OAUTHAP`);

  // opens the url in the default browser 
  open(`https://api.tdameritrade.com/v1/oauth2/token?client_id=${clientId}&response_type=code&redirect_uri=${encodeURI(redirectURI)}`);
